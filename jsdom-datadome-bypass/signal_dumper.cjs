/**
 * Run the hooked interstitial HTML in jsdom and dump all captured signals.
 * 
 * Usage: node signal_dumper.cjs <hooked_html_path> <interstitial_url> <target_url>
 */
const { JSDOM } = require('/home/ubuntu/node_modules/jsdom');
const { createCanvas } = require('/home/ubuntu/node_modules/@napi-rs/canvas');
const fs = require('fs');

const htmlPath = process.argv[2] || '/tmp/dd_challenge_hooked.html';
const interstitialUrl = process.argv[3] || 'https://geo.captcha-delivery.com/interstitial/';
const targetUrl = process.argv[4] || 'https://www.truepeoplesearch.com/';
...
const dom = new JSDOM(html, {
    ...
    beforeParse(window) {
        ...
        Object.defineProperty(window, 'screen', {
            value: { width: 1920, height: 1080, availWidth: 1920, availHeight: 1040, colorDepth: 24, pixelDepth: 24, orientation: { type: 'landscape-primary', angle: 0, addEventListener: () => {} } }, configurable: true
        });
        ...
        for (const [k, v] of Object.entries(winProps)) Object.defineProperty(window, k, { value: v, configurable: true });
        ...
        window.document.createElement = function(tag) {
            ...
            if (tag.toLowerCase() === 'canvas') {
                ...
                function getRealCanvas() {
                    if (!_realCanvas || _realCanvas.width !== _w || _realCanvas.height !== _h) _realCanvas = createCanvas(_w, _h);
                    return _realCanvas;
                }
        ...
        function makeWebGL(canvas) {
            const params = {
                37445: 'Google Inc. (NVIDIA)', 37446: 'ANGLE (NVIDIA, NVIDIA GeForce GTX 1060 6GB Direct3D11 vs_5_0 ps_5_0, D3D11)',
                7938: 'WebGL 1.0 (OpenGL ES 2.0 Chromium)', 7936: 'WebKit', 7937: 'WebKit WebGL',
                3379: 16384, 34076: 16384, 3386: new Int32Array([32767, 32767]),
        ...
        window.XMLHttpRequest = function() {
            ...
            xhr.send = function(body) {
                if (body && _url.includes('interstitial')) {
                    ...
                    // Dump signals from the window context
                    const signals = dom.window._ddSignals || [];
                    fs.writeFileSync('/tmp/dd_signals.json', JSON.stringify(signals, null, 2));
                    console.log('SIGNALS_DUMPED:' + signals.length);
                }
...
setTimeout(() => { try { dom.window.close(); } catch(e) {} process.exit(0); }, 15000);
