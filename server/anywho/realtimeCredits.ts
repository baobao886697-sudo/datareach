 ...
 * - 费用明细统计
 */
export class AnywhoRealtimeCreditTracker {
  ...
  private async deduct(
    ...
    try {
      ...
      const description = type === 'search' 
        ? `Anywho搜索页 [${this.taskId}] - ${roundedAmount} 积分`
        : `Anywho详情页 [${this.taskId}] - ${count}条 × ${this.detailCost} = ${roundedAmount} 积分`;
      ...
      console.error(`[Anywho] 积分扣除失败:`, error);
      return {
        success: false,
        newBalance: this.currentBalance,
        deductedAmount: 0,
 ...
 * 创建 Anywho 实时积分跟踪器
 */
export async function createAnywhoRealtimeCreditTracker(
  userId: number,
  taskId: string,
  searchCost: number,
  detailCost: number
): Promise<AnywhoRealtimeCreditTracker> {
  const tracker = new AnywhoRealtimeCreditTracker(userId, taskId, searchCost, detailCost);
 ...
 * 格式化费用明细为日志字符串
 */
export function formatAnywhoeCostBreakdown(
